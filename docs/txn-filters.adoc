...
For tackler cli usage of transaction filters, see link:./usage.adoc[Usage Guide].
...
* `OR`: if any of filters selects txn, then it's selected
* `NOT`: reverses result (selected txn will be unselected, unselected to be selected)
