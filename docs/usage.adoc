...
then it is good idea to use as specialized `input.txn.dir` as possible. In that case scanned file set is as small as possible.
...
These options are mutually exclusive with each other and with filesystem storage arguments (`input.txn.*`).
